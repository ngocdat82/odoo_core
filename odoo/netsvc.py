...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('odoo.sql_db'), sql_db.db_connect(dbname,
...
def init_logger():
    global _logger_init, last_log_time, handler  # Thêm biến global last_log_time và handler
    if _logger_init:
        return
    _logger_init = True
    ...
    if tools.config['syslog']:
        ...
        # Đặt thời gian khởi tạo log ban đầu
        dt = datetime.now(pytz.timezone('Asia/Ho_Chi_Minh'))
        last_log_time = dt  # Thêm biến này để lưu trữ thời gian log

        def create_log_handler():
            global last_log_time, handler

            dt = datetime.now(pytz.timezone('Asia/Ho_Chi_Minh'))
            log_time = dt.replace(hour=0, minute=0, second=0, microsecond=0)  # Làm tròn về đầu ngày

            if log_time != last_log_time:  # Nếu ngày đã thay đổi, tạo file log mới
                logf_new = logf + "/" + log_time.strftime("%Y%m%d") + "_" + "1erp-15.log"
                last_log_time = log_time  # Cập nhật thời gian log (theo ngày)

                # Xóa handler cũ và tạo handler mới
                if handler:
                    logging.getLogger().removeHandler(handler)

                if not os.path.exists(logf_new):
                    dirname = os.path.dirname(logf_new)
                    if dirname and not os.path.isdir(dirname):
                        os.makedirs(dirname)

                if os.name == 'posix':
                    handler = logging.handlers.WatchedFileHandler(logf_new)
                else:
                    handler = logging.FileHandler(logf_new)

                logger = logging.getLogger()
                logger.addHandler(handler)
                logger.setLevel(logging.INFO)

        create_log_handler()  # Tạo log lần đầu

        def log_rotation():
            while True:
                create_log_handler()  # Kiểm tra mỗi lần ghi log xem có cần tạo log mới không
                time.sleep(10)  # Kiểm tra mỗi 10 giây xem có cần tạo log mới không

        import threading
        log_thread = threading.Thread(target=log_rotation, daemon=True)
        log_thread.start()
...
#         # log chi ghi theo phut ok
#         from datetime import datetime
#         import pytz
#         dt = datetime.now(pytz.timezone('Asia/Ho_Chi_Minh'))
#         logf = logf + "/" + dt.strftime("%Y%m%d") + "_" + "1erp-15.log"
#
#         try:
#             # Check log của ngày hôm nay đã tồn tại chưa
#             if not os.path.exists(logf):
#                 # Check vị trí lưu file
...
#                 logger.setLevel(logging.INFO)
#             else:
#                 # Nếu file log của ngày hôm nay đã tồn tại thì tiếp tục sử dụng file này **content by dat.dongoc**
#                 handler = logging.FileHandler(logf)
#                 logger = logging.getLogger()
#                 logger.addHandler(handler)
#                 logger.setLevel(logging.INFO)
