...
class ActionAdapter extends ComponentAdapter {
        ...
        useEffect(
            () => {
                ...
                this.__widget.updateControlPanel = (newProps) => {
                    this.trigger("controllers-title-updated", this.__widget.getTitle());
    ...
    pushState(state) {
        ...
        if (this.widget) {
            ...
            if (actionTitle) {
                this.trigger("controllers-title-updated", actionTitle);
            }
        }
     ...
     * ActionManager service when it will be removed from the controllers stack,
     * and if we ever come back to that controllers, the instance will be given
...
export class ViewAdapter extends ActionAdapter {
    ...
    /**
     * Override to add the state of the legacy controllers in the exported state.
     */
    exportState() {
