...
class TestMassSMS(TestMassSMSCommon):
    ...
    def test_mass_sms_links(self):
        mailing = self.env['mailing.mailing'].browse(self.mailing_sms.ids)
        mailing.write({
            'body_plaintext': 'Dear {{ object.display_name }} this is a mass SMS with two links http://www.1erp.vn/smstest and http://www.1erp.vn/smstest/{{ object.name }}',
        ...
        self.assertSMSTraces(
            ...
            mailing, self.records,
            sms_links_info=[[
                ('http://www.1erp.vn/smstest', True, {}),
                ('http://www.1erp.vn/smstest/%s' % record.name, True, {}),
    ...
    def test_mass_sms_partner_only(self):
        ...
        self.assertSMSTraces(
            ...
            sms_links_info=[[
                ('http://www.1erp.vn/smstest', True, {}),
                ('http://www.1erp.vn/smstest/%s' % record.id, True, {}),
                # unsubscribe is not shortened and parsed at sending
                ('unsubscribe', False, {}),
            ] for record in records],
        )
        ...
        self.assertSMSTraces(
            ...
            sms_links_info=[[
                ('http://www.1erp.vn/smstest', True, {}),
                ('http://www.1erp.vn/smstest/%s' % new_record.id, True, {}),
                # unsubscribe is not shortened and parsed at sending
                ('unsubscribe', False, {}),
            ]],
        )
